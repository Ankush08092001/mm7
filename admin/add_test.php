...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        // Create new test
        $stmt = $pdo->prepare("
            INSERT INTO mock_tests (title, description, duration, passing_score, is_active, created_at)
            VALUES (?, ?, ?, ?, ?, NOW())
        ");
        
        if ($stmt->execute([$title, $description, $duration, $passing_score, $is_active])) {
...
<head>
    ...
    <title>Add Mock Test - Admin Portal</title>
    ...
    <div class="admin-container">
        ...
        <main class="admin-content">
            <header class="admin-header">
                <h1 class="admin-header-title">Add New Mock Test</h1>
                ...
                <form method="POST" class="admin-form">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="duration" name="duration" class="form-control" 
                               value="<?php echo htmlspecialchars($_POST['duration'] ?? '60'); ?>" min="1" required>
                    </div>

                    <div class="form-group">
                        <label for="passing_score" class="form-label">Passing Score (%)</label>
                        <input type="number" id="passing_score" name="passing_score" class="form-control" 
                               value="<?php echo htmlspecialchars($_POST['passing_score'] ?? '60'); ?>" min="0" max="100" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">
                            <input type="checkbox" name="is_active" <?php echo isset($_POST['is_active']) ? 'checked' : ''; ?>>
                            Active Test
                        </label>
                    </div>
